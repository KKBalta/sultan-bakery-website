...
// This service handles fetching Google Reviews with proper error handling and fallbacks
...
export class GoogleReviewsService {
  ...
  /**
   * Fetch Google Reviews with caching and error handling
   */
  async fetchReviews(
    placeId: string,
    apiKey: string,
    ...
    try {
      ...
      const reviews = await this.fetchFromGoogleAPI(placeId, apiKey, maxReviews);
   ...
   */
  private async fetchFromGoogleAPI(
    placeId: string,
    apiKey: string,
    maxReviews: number
    ...
    return data.result.reviews.slice(0, maxReviews).map((review: any) => ({
